...
interface WithdrawActionCardProps {
  address: string;
  sm?: true;
...
const WithdrawActionCard: FC<WithdrawActionCardProps> = ({
  address,
  balance,
  symbol,
  txHash,
    ...
    <WalletCardBox w={"full"} px={4} py={2}>
      ...
      >
        ...
        <VStack alignItems={"end"}>
          <HStack fontSize={"sm"} fontWeight={"semibold"} color={"lightcoral"}>
            <Text>- {numberWithCommaSeparate(balance)} </Text>
            <Text>{symbol} </Text>
          </HStack>
          {/* <Text fontSize={"xs"} fontWeight="normal">
            {convertReferenceToBetterNames()}{" "}
          </Text> */}
        </VStack>
      </HStack>
    </WalletCardBox>
